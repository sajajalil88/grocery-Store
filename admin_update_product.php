...
if(isset($_POST['update_product'])){
    ...
    $update_product = mysqli_query("UPDATE 'products' SET name = '$name', category = '$category', details = '$details', price ='$price'? WHERE id = '$pid'");
    ...
    if(!empty($image)){
        if($image_size > 2000000){
           ...
           $update_image = mysqli_query($conn,"UPDATE `products` SET image = '$image' WHERE id = '$pid'");
...
<body>
    ...
    <?php
     $update_id = $_POST['update_product'];
    $update_product = mysqli_query($conn , "SELECT * FROM 'products' WHERE id = '$update_id'")  or die('query failed');
    if(mysqli_num_rows($update_product) > 0 )
    {
        while($fetch_products = mysqli_fetch_assoc($update_product)){
